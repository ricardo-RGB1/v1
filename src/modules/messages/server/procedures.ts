...
export const messagesRouter = createTRPCRouter({
   ...
   *
   * Process:
   * 1. Consumes user credits to validate they have remaining usage allowance
   * 2. Creates a new USER message in the database with type "RESULT"
   * 3. Triggers the "code-agent/run" Inngest event to process the user's request
   * 4. Returns the created message object
   *
   * Returns: The newly created message object with id, content, role, type, etc.
   * 
   * Throws: 
   * - TRPCError with "NOT_FOUND" code if project doesn't exist
   * - TRPCError with "BAD_REQUEST" code if credit consumption fails unexpectedly
   * - TRPCError with "TOO_MANY_REQUESTS" code if user has exceeded their credit limit
   */
      ...
      try {
        ...
        if (error instanceof Error) {
          ...
          throw new TRPCError({
            ...
            message:
              "You have reached the maximum number of requests. Please upgrade to a paid plan.",
          });
        }
      }
